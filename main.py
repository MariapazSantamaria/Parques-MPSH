...
def Gra():
    ...
    LF.img = fondo
    LF.pack()
    LB = Label(Pan, text = "WELCOME TO PARCHIS", font = ("Times New Roman", 30))
    LB.place(x = 10, y = 10)
    if (OS == "Windows"):
        BTN = Button(Pan, text = "JUGAR", command = lambda: threading.Thread(target=IniciarJuego).start(),
                     font=("Algerian", 40))
    else:
        BTN = Button(Pan, text="JUGAR", command=IniciarJuego, font=("Algerian", 40))
...
def CrearTablero():
    ...
    lab=""
    if mod>1:
        imagen=PhotoImage(file="winner.gif")
...
def pedirDatos():
    ...
    n = 0
    if mod>1:
        Esp = Label(Pan, text="INGRESA LA CANTIDAD DE JUGADORES", bg="white", fg="black", font=("Times New Roman", 25))   
        Esp.place(y=0, x=0)
        Esp2 = Label(Pan, text="MAXIMO DE 4 JUGADORES", bg="white", fg="black", font=("Times New Roman", 25))   
        Esp2.place(y=100, x=0)
        if mod<3:
    ...
    if mod==3:
        caja_entrada_dados.delete(0,END)
        caja_entrada_dados.insert(0,"Presione continuar")
        texto_ingresado=""
        while x=="":
            x=texto_ingresado
    else:
...
def OrdenDeJuego(ListaJugadores, modoDesarrollador=False):
    ...
    while (len(aux) != 1):
        ...
        aux=aux2
    if mod<3:
        print(aux[0].nombre + " ES EL PRIMERO EN JUGAR")
    else:
        tkinter.messagebox.showinfo("Aviso",aux[0].nombre + " ES EL PRIMERO EN JUGAR")
...
while(ele<=0 or ele>3):
  print("WELCOME TO PARCHIS:\nSeleccione una de las siguientes:\n1.Modo Consola\n2.Modo Consola junto con Interfaz Grafica\n3.Interfaz Grafica\n")
  ele=input()
  try:
    ele=int(ele)
  except:
